...
def form_long_sequences(df):
    """ Helper function to combine sentences and build long sequences thus induce instable gradients flow along time steps"""
    rows = []
    counter, max_cntr = 0, 20
    curr_row = {"en" : "", "de": ""}
...
class NMTDataset(Dataset):
    def __init__(self, df):
        # self.df = form_long_sequences(df)
        self.df = df
    def __len__(self):
        return len(self.df)
...
class TranslationV0(nn.Module):
    ...
    def forward(self, encoder_inp, decoder_inp, enc_ip_lengths, dec_ip_lengths):
        ...
        # self.encoder_outputs = out
        # self.encoder_outputs.retain_grad() #Even setting retain grad will not technically work since encoder outputs are not
        # sent to decoder, only the hidden state is passed so backward flow of gradients does not flow through encoder outputs
        # so the change is pool the outputs and send to decoder instead of hidden -> this stimulation is purely to understand flow of gradients in encoder
        # hidden = self.encoder_outputs.mean(dim=1).unsqueeze(dim=0)
...
def bleu_wrapper(logits, y_batch_target, lengths, vocab_map):
...
if __name__ == "__main__":
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        grad_norms = []
        model.train()
        for batch_idx, (X_batch, y_batch_ip, y_batch_tar, enc_lengths, dec_lengths) in enumerate(train_loader):
            logits = model(X_batch, y_batch_ip, enc_lengths, dec_lengths)
            # print(f"[DEBUG] Does Encoder Outputs Tensor exist : {model.encoder_outputs.requires_grad}, "
            #       f"Does Encoder Outputs have grad fn {model.encoder_outputs.grad_fn}") #This proved Tensor exist and has grad fn
            B, S, V = logits.shape
            loss = loss_fn(logits.view(-1, V), y_batch_tar.view(-1))
            acc = bleu_wrapper(logits, y_batch_tar, dec_lengths, id_to_de)
            optimizer.zero_grad()
            loss.backward()
            # print(f" [DEBUG] Did the Encoder outputs tensor participate in autograd : {model.encoder_outputs.grad}") #However the gradient is None proving no backward flow

            #-------------------------- Plot gradient across time steps  -----------------------------------
            # Check for gradients in each time step of the encoder model
            # grad = model.encoder_outputs.grad
            # grad_norm_per_timestep = grad.norm(dim=2).mean(dim=0).detach().cpu()
            # flow = "encoder"

            # Given a gradient tensor from the encoder hidden states with shape (B, S, H) — example 10, 5, 8 meaning 10 samples in a batch each holding 5 time step and each time step is a 8-dim vector
            # — we first compute grad.norm(dim=2) to measure the magnitude of the gradient vector( L2 norm) for each hidden state vector.
            # So the shape becomes 10,5,8 to 10, 5 where each  sample now has a single gradient magnitude for each time step
            # Next, we compute .mean(dim=0) to average these gradient magnitudes across all samples in the batch, reducing the tensor to shape (S,).
            # The resulting vector contains one value per timestep, representing the average gradient strength flowing through that encoder timestep across the batch,
            # which allows us to visualize whether gradients diminish for earlier timesteps (indicating vanishing gradients).
        ...
        # Insights based on the gradient tracking process :
        # Problem we are facing right now is
        # Vanilla RNN Seq2Seq ->  Encoder gradients vanish -> Decoder dominates learning ->  Poor translations

        # Architecture	        Gradient behaviour
        # Final hidden state	vanishing gradient
        # Mean pooling	        uniform gradients
        # Attention	            selective gradients
